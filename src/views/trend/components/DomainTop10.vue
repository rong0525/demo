...
export default{
        ...
        option : {
          ...
          axisLabel: {        
              formatter: function(value) {       
                ...
                let exponent = Math.floor(Math.log10(value));  // 获取 10 的幂指数
                let coefficient = value / Math.pow(10, exponent);  // 获取系数
                // const exponentStr = exponent.toString().split('').map(digit => `\u00b${digit}`).join('');
                // 返回系数和 10 的幂次方，以角标的形式显示
                return `${coefficient.toFixed(0)}×10^${exponent}`;      
                // return `${coefficient.toFixed(0)}×10${exponentStr}`;
              }    
              }
           },
          yAxis: { 
          ...
          series: [
            {
              ...
              label: {
                ...
                position: 'inside' // 设置标签显示在柱状图顶部，也可以设置为 'inside' 等其他位置
            },
              encode: {
    ...
    methods:{
        ...
        getData(){
          axios.get('http://10.127.220.44:9292/getDohTimesTop10')
          .then(response => {
