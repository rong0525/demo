...
export default{
    ...
    methods:{
        ...
        async getData(){
          const dohresponse = await axios.get('http://localhost:9292/getDohSeStability');
          const dotresponse = await axios.get('http://localhost:9292/getDoqSeStability');
          const doqresponse = await axios.get('http://localhost:9292/getDotSeStability');
        ...
        classifyData(data) {
          const thresholds = {
            ...
            data.forEach((number) => {
                if (number < lowThreshold && number>0) {
                    result.low++;
            ...
            result.low=((result.low+result.high*0.013)/sum).toFixed(3);
