<template>
  <div style="width: 100%">
    <el-table
      ...
      height="550px"
    >
      <el-table-column label="label" align="left" prop="label" width="60">
        <template slot-scope="{row}">
          <span>{{ row.label | labelFilter}}</span>
        </template>
      </el-table-column>

      <el-table-column label="ImageFeature" align="center" width="120" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <img :src="row.imageFeature" width="36px" height="36px">
        </template>
      </el-table-column>

      <el-table-column label="Feature" align="left" width="10000" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
...
export default {
  ...
  watch:{
    step(newval){
      if (newval === '2'){
        this.getDataTimes = 1
        this.getList(this.getDataTimes, this.getDataTimes * 10)
        this.timer = setInterval(() => {
          if (this.getDataTimes < 40) {
