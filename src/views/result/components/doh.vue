<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="IP" width="320px" align="center" >
        <template slot-scope="{row}">
          <span>{{ row.serverip }}</span>
        </template>
      </el-table-column>

      <el-table-column label="域名" width="600px" align="center" >
        <template slot-scope="{row}">
...
export default {
  ...
  methods: {
    getList() {
      ...
      feature.get('/hscan/all', params).then(res => {
    ...
    handleFilter(){
      ...
      feature.get('/hscan/search',params).then(res => {
          this.list = res.data
          console.log("total", this.total);
          this.total = res.total
          this.listLoading = false
